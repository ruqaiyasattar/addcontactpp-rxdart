import 'dart:convert';
import 'dart:io';
import 'package:rxdart_demoapp/models/animals.dart';
import 'package:rxdart_demoapp/models/person.dart';
...
class Api {
  ...
  Future<List<Thing>> search(SearchTerm searchTerm) async {
    ...
    final persons = await _getJson('http://192.168.2.108:8080/persons.json')
        .then((json) => json.map((value) => Person.fromJson(value)));
    _persons = persons.toList();

    // start by calling persons api
    final animals = await _getJson('http://192.168.2.108:8080/animals.json')
